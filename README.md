...
| **Agent Framework** | LangChain v1.0, LangGraph (`create_react_agent`) |
...
│   ├── agents/
│   │   ├── medical_agent.py            # ReAct 에이전트 팩토리 (create_react_agent)
│   │   ├── prompts.py                  # 시스템 프롬프트 (도구 설명, 답변 규칙)
│   │   └── tools.py                    # 5개 도구 정의 + ES Retriever + 유틸리티
│   ├── api/
│   │   └── routes/
...
| 모듈 | 역할 |
|------|------|
| `agents/medical_agent.py` | `create_react_agent()`로 LangGraph 에이전트를 생성. LLM + 도구 목록 + 시스템 프롬프트 + 체크포인터를 조합 |
| `agents/tools.py` | 5개 `@tool` 데코레이터 함수 정의. ES 클라이언트/Retriever 설정, 공공데이터 API 호출, XML/JSON 파싱, 지역/진료과목 코드 매핑 |
