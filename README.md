...
LangGraph StateGraph 기반 검색 서브에이전트가 Elasticsearch에서 BM25 + Vector kNN 하이브리드 검색 → Cohere ReRank를 수행하여 38,000건 이상의 의료 문서와 818건의 의약품 PDF를 검색하고, 공공데이터포털 REST API를 통해 병원/의약품/응급실/약국 정보를 실시간으로 조회합니다.
...
| **Agent Framework** | LangChain v1.2 (`create_agent`, `ToolStrategy`) |
| **LLM** | OpenAI GPT-4o (`langchain-openai`) |
| **검색 엔진** | Elasticsearch 8.x (BM25 + Vector kNN 하이브리드) |
| **검색 서브에이전트** | LangGraph StateGraph (BM25 ∥ Vector → 병합 → Cohere ReRank) |
| **ES 연동** | `elasticsearch` (직접 클라이언트) |
| **ReRank** | Cohere `rerank-v3.5` (선택, fallback: score 정렬) |
| **외부 API** | 공공데이터포털 REST API (4개 서비스) |
| **HTTP Client** | httpx (비동기) |
| **설정 관리** | pydantic-settings (`.env` 기반) |
| **관측성** | Opik (LangGraph 트레이싱) |
| **패키지 관리** | uv |
| **Python** | 3.11 ~ 3.13 |
...
3. `AgentService`가 매 요청마다 에이전트를 생성하고 `astream(stream_mode="updates")`으로 중간 이벤트를 스트리밍합니다.
4. 에이전트는 LLM이 판단한 도구를 실행하고, 각 단계를 SSE(Server-Sent Events)로 실시간 전달합니다.
5. `ToolStrategy(ChatResponse)`로 구조화된 최종 응답을 생성합니다. astream에서 done이 안 나오면 LLM 직접 호출 fallback으로 보장합니다.
6. chat.py에서 SSE heartbeat를 보내 장시간 응답 시에도 연결을 유지합니다.
...
### create_agent vs StateGraph 기술 선택

`create_agent`는 내부적으로 LangGraph의 `StateGraph`를 구성하는 고수준 API입니다. 표준 ReAct 패턴(model → tools → model 반복 → 최종 응답)으로 충분한 경우 적합하며, `middleware` 파라미터로 횡단 관심사를 확장할 수 있습니다.

커스텀 라우팅(도구 종류별 분기, 최종 응답 전 후처리 노드 추가 등)이 필요한 경우 `StateGraph` 직접 구성으로 전환할 수 있습니다. 현재 프로젝트는 표준 ReAct 패턴으로 충분하므로 `create_agent`를 사용합니다.

## 에이전트 도구 (Tools)

에이전트는 사용자 질문을 분석하여 아래 5개 도구 중 적절한 것을 자동으로 선택합니다.

### Tool 1: `search_medical_info` - 의료 문서 하이브리드 검색 (서브에이전트)

| 항목 | 내용 |
|------|------|
| **구현** | LangGraph StateGraph 기반 서브에이전트 (`search_agent.py`) |
| **검색 방식** | BM25(`edu-collection`, 38,154건) + Vector kNN(`edu-medicine-info`, 818건) 병렬 → 병합 → Cohere ReRank |
| **입력** | `query`: 증상, 질병명, 치료법 등 (예: `"결핵 치료"`, `"천식 증상"`) |
| **출력** | 상위 5개 문서 (relevance score, 출처, 본문 500자) |
| **문서 출처** | WHO, wikidoc, KSEM, 대한간학회, 대한결핵및호흡기학회, 식약처 의약품 PDF 등 |
...
│   ├── agents/
│   │   ├── medical_agent.py            # 에이전트 팩토리 (ToolStrategy + @dataclass ChatResponse)
│   │   ├── search_agent.py            # 검색 서브에이전트 (BM25+Vector→ReRank StateGraph)
│   │   ├── middleware.py               # @wrap_tool_call 에러 처리 middleware
│   │   ├── prompts.py                  # 시스템 프롬프트 (도구 설명, 답변 규칙)
│   │   ├── region_codes.py             # 전국 시도/시군구 코드 매핑 + parse_region()
│   │   └── tools.py                    # 4개 도구 정의 (async) — 병원/의약품/응급실/약국
│   ├── api/
│   │   └── routes/
...
| 모듈 | 역할 |
|------|------|
| `agents/medical_agent.py` | `create_agent()`로 ReAct 에이전트 생성. `ToolStrategy(ChatResponse)` + `@dataclass` 기반 응답 스키마. model을 외부에서 주입받음 |
| `agents/search_agent.py` | LangGraph StateGraph 기반 검색 서브에이전트. BM25 + Vector kNN 병렬 → 병합 → Cohere ReRank. `@tool`로 래핑 |
...
| `services/agent_service.py` | 매 요청마다 에이전트 재생성. astream(중간 progress) + fallback(done 보장) 하이브리드. `MemorySaver` 체크포인터. GraphRecursionError/체크포인트 정합성 처리. Opik 트레이싱 |
| `api/routes/chat.py` | SSE `StreamingResponse` + heartbeat(3초). AgentService 싱글턴 |
