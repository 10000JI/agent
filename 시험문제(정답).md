...
**Bug 1**: `MemorySaver()`가 아닙니다.
LangChain v1 문서의 short-term memory 예시는 `InMemorySaver()` 를 사용합니다.

**Bug 2**: `@tool` 데코레이터를 클래스의 인스턴스 메서드(`self`가 있는 메서드)에 직접 사용하고 있습니다. LangChain tool은 일반 함수 형태로 정의하거나 `self`가 없는 static 형태로 작성해야 합니다. 따라서 `search_symptom`을 클래스 외부 함수로 분리하거나 `self`를 제거해야 합니다.

**Bug 3**: `response_fromat`은 오타이며 `response_format`으로 수정해야 합니다.
...
**문제점**: `process_query()`가 호출될 때마다 `InMemorySaver()`를 새로 생성합니다. 새 인스턴스에는 이전 대화 기록이 없기 때문에, thread_id를 넘겨도 이전 대화를 찾을 수 없습니다. 결국 매번 새 대화가 시작되어 멀티턴이 동작하지 않습니다.
...
class AgentService:
    def __init__(self, model, tools):
        self.agent = create_agent(
            model=model,
            tools=tools,
            checkpointer=InMemorySaver()  # 인스턴스당 1회 생성
        )
...
**(1)** 문제점: 없음
`from langchain.agents.structured_output import ToolStrategy` 경로를 직접 사용합니다.

**(2)** 문제점: 없음
`from langchain.tools import tool`을 권장 경로로 사용합니다.

**(3)** 상태: 정상.
`langchain_core.messages`는 메시지 타입의 원본 위치입니다. v1에서 `langchain.messages`로도 접근 가능하지만 이 경로도 유효합니다.

**(4)** 상태: 정상.
v1 에이전트 팩토리 함수입니다.

**(5)** 상태: 정상.
정상이나 중복입니다. `langchain_core.tools`은 (2) `langchain.tools`의 원본 구현 위치입니다. 동일한 객체를 가리키므로 둘 다 유효하지만, v1 기준 권장은 (2)의 `langchain.tools` 경로입니다.
...
`response_format=ToolStrategy(ChatResponse)`는 에이전트의 최종 응답을 특정 구조(ChatResponse)로 반환하도록 지정하는 structured output 방식입니다. 반면 `ChatResponse`를 `@tool`로 정의하면 출력 스키마가 아니라 모델이 호출할 수 있는 일반 도구가 됩니다. 공식 API 관점에서는 `response_format`을 사용하는 structured output 방식이 권장됩니다. 이 방식은 모델 응답을 스키마로 검증할 수 있어 응답 형식의 안정성과 일관성이 높기 때문입니다.
