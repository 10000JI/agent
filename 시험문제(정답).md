...
**Bug 1**: `MemorySaver`가 import 되어 있지 않습니다. `from langgraph.checkpoint.memory import InMemorySaver`를 추가하고 `InMemorySaver()`로 변경해야 합니다.
...
**(1)** 문제점: `langchain.agents.structured_output` 모듈의 `ToolStrategy`는 버전에 따라 존재하지 않을 수 있는 경로입니다. 최근에 추가된 API이므로 버전 호환성이 보장되지 않습니다.
올바른 경로: `from langchain.agents import ToolStrategy`

**(2)** 문제점: langchain.tools 경로는 deprecated 되었으며 v1 기준으로 langchain_core에서 제공됩니다. -> 올바른 경로: `from langchain_core.tools import tool`
